...
$env:OPENCODE_CONFIG = $ConfigPath  # pin serve + runners to CICADA config - never inherit the global opencode cascade
$ConsoleId = [guid]::NewGuid().ToString("N").Substring(0,8); $WorkersFile = Join-Path $Root ("state\workers-" + $ConsoleId + ".json")
...
$w.task = $flat; $w.state = "busy"; $w.log = $log; $w.updated = [DateTime]::UtcNow.ToString("o"); Save-WorkerRegistry
...
function Get-WorkerTail([string]$id) {
    ...
    if (-not $text) { Write-Host "  (turn completed, but the agent produced no text response - inspect /tail or the worker log)" -ForegroundColor Yellow; return }
    $lines = $text -split "`n"
...
function Interrupt-Worker([string]$id, [string]$note) {
$w = $script:WorkerRegistry[$id]
if (-not $w) { Write-Host ("  No worker " + $id + ".") -ForegroundColor Red; return }
$entry = $script:Jobs[$id]
if (-not $entry) { Write-Host ("  worker " + $id + " is idle - sending your note as a normal message") -ForegroundColor DarkGray; Send-WorkerMessage $id $note; return }
Write-Host ("  interrupting worker " + $id + " - turn cancelled server-side; session and all work retained") -ForegroundColor Yellow
Abort-WorkerTurn $id
$deadline = (Get-Date).AddSeconds(15)
while ($script:Jobs.ContainsKey($id) -and (Get-Date) -lt $deadline) {
Finalize-WorkerJob $id
if (-not $script:Jobs.ContainsKey($id)) { break }
Start-Sleep -Milliseconds 500
}
Finalize-WorkerJob $id
$advice = "INTERRUPT FROM THE OPERATOR (this is not a new task - retain exactly where you were and what you had done): " + $note + " Take this on board, briefly confirm what you will change, then continue your work with it applied."
Send-WorkerMessage $id $advice
}
...
Write-Host "  /interrupt <n> <msg> pause worker n mid-turn, give it advice - it retains all work and continues"
...
# ---------- CICADA working-aware REPL ----------
function Get-BusyWorkerIds {
    return @($script:Jobs.Keys | Sort-Object { [int]$_ })
}

function Write-CicadaReplPrompt([string]$Line = "") {
    $busyIds = @(Get-BusyWorkerIds)
    if ($busyIds.Count -gt 0) {
        $labels = @($busyIds | ForEach-Object { "worker " + $_ }) -join ", "
        Write-Host ("control [" + $labels + " working - /status /doing <n> /abort <n> /interrupt <n> <msg>]: " + $Line) -NoNewline -ForegroundColor Yellow
    } else {
        Write-Host ("cicada: " + $Line) -NoNewline -ForegroundColor DarkGray
    }
}

function Redraw-CicadaReplPrompt([string]$Line = "") {
    $width = 120
    try { $width = [math]::Max(80, [Console]::WindowWidth - 1) } catch {}
    [Console]::Write("`r" + (" " * $width) + "`r")
    Write-CicadaReplPrompt $Line
}
# ---------- setup ----------
...
# ---------- optional working file: its contents ride along with every worker task ----------
if (-not $TaskFile -and -not $Yes) {
    $TaskFile = (Read-Host "Working file for the workers (path, or Enter to skip)").Trim()
}
$fileContext = ""
if ($TaskFile) {
    if (Test-Path $TaskFile) {
        $fileContext = (Get-Content $TaskFile -Raw).Trim()
        Write-Host ("working file loaded: " + $TaskFile + " (" + $fileContext.Length + " chars)") -ForegroundColor Green
    } else {
        Write-Host ("working file not found: " + $TaskFile + " - continuing without it") -ForegroundColor Yellow
    }
}
if ($fileContext) {
    for ($i = 0; $i -lt $taskList.Count; $i++) {
        if ($taskList[$i]) {
            $taskList[$i] = $taskList[$i] + " Refer to the working file below - its full contents are included here. WORKING FILE (" + $TaskFile + "): " + $fileContext
        }
    }
}

# ---------- plain-English reply style: workers answer in prose, not reports ----------
$replyStyle = " When you reply, use plain English prose: a short paragraph saying what you did, which files you created, edited, or deleted (name them in the sentence), what checks you ran and whether they passed, and anything I need to know before this ships. No headings, no bullet sections, no tables - just a clear paragraph or two."
for ($i = 0; $i -lt $taskList.Count; $i++) { if ($taskList[$i]) { $taskList[$i] += $replyStyle } }
$Model = Resolve-ModelName $Model
Show-ReplayCommand -ParamNames @("Project","Workers","Model","Agent","Tasks","TaskFile","Yes")
...
try {
    while ($true) {
        Sweep-Jobs
        Write-CicadaReplPrompt
        $line = ""
        while ($true) {
            Sweep-Jobs
            if ($script:ConsolePrinted) { Redraw-CicadaReplPrompt $line; $script:ConsolePrinted = $false }
        ...
        if ($line -match "^(\d+)\s*>\s*(.+)$") {
            ...
            Send-WorkerMessage $Matches[1] ($Matches[2] + " (Reply in plain-English prose - a short paragraph, no headings or bullet sections.)")
            continue
        }

if ($line -match "^/(interrupt|pause|nudge)\s+(\d+)\s+(.+)$") {
    Interrupt-Worker $Matches[2] $Matches[3]
    continue
}
        if ($line -match "^/(\w+)(?:\s+(\d+))?$") {
            ...
            elseif ($cmd -eq "doing") { if ($n -and $script:WorkerRegistry[$n]) { $act = Get-WorkerCurrentAction $script:WorkerRegistry[$n].log; Write-Host ("  worker " + $n + ": " + $(if ($act) { $act } else { "idle - nothing in flight" })) -ForegroundColor Cyan } else { Write-Host "  usage: /doing <n>" -ForegroundColor DarkGray } }
        ...
        if ($line.StartsWith("/")) {
            Write-Host "Malformed command - enter one slash command per line (for example: /status or /doing 1)." -ForegroundColor Yellow
            continue
        }
